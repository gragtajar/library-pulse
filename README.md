...
1. **Figma Plugin** — runs inside Figma; handles OAuth flows, file selection, and channel configuration.
...
| Figma | OAuth App | [figma.com/developers](https://www.figma.com/developers) |
...
1. Go to [figma.com/developers](https://www.figma.com/developers) and create a new app.
2. Set the **Callback URL** to:
   ```
   https://YOUR-VERCEL-DOMAIN/api/auth/figma-callback
   ```
3. Request these scopes: `files:read`, `webhooks:write`
...
2. **Connect Figma** — OAuth flow authorizes webhook registration.
...
5. **Save & Activate** — the backend automatically registers a Figma webhook for your team.

### When a library is published

1. Figma fires a `LIBRARY_PUBLISH` webhook event.
2. The backend looks up all active configurations for that file.
...
- **Figma OAuth tokens** are similarly encrypted at rest.
...
2. **Team-level webhooks**: Figma webhooks fire for all files in a team. The handler filters by file key — other files are ignored silently.

3. **One webhook per team**: Multiple users on the same team share a single webhook registration. The first user to activate triggers the registration.
...
│   │   │   └── figma-callback.js  Figma OAuth callback
